...
```
def find_cube_pairs(targ): # target changed to targ, : (colon) added
    ...
    for a in range(1, max_num + 1): # : added, changed ranges to range
        for b in range(a, max_num + 1): # : (colon) added, changed ranges to range
            if a**3 + b**3 == targ: # 3 * changed to 2 * and : (colon) added
